...
SD2 / Kandinsky autoencoders) for every image in data/real/, using both T4 GPUs
concurrently. Writes results/real_scores.csv. Does NOT touch generated/.
...
The real/ image list is split `[part::num_parts]` between the two GPUs (interleaved,
so both get an even, representative half).
...
def main():
    ...
    real_dir = Path(args.data_root) / "real"
    images = _iter_images(real_dir)[args.part::args.num_parts]
    print(f"[{device}] part {args.part}/{args.num_parts}: {len(images)} real images", flush=True)
    ...
    with open(args.out, "w", newline="") as f:
        ...
        for i, path in enumerate(images, 1):
            ...
            if i % 200 == 0 or i == len(images):
                ...
                eta = (len(images) - i) / rate if rate > 0 else float("inf")
                print(
                    f"[{device}] {i}/{len(images)}  {path.name}: {score:.4f}  "
...
def main():
    ...
    worker_path = Path("_aeroblade_real_worker.py")
    worker_path.write_text(WORKER_SOURCE)

    part_paths = [RESULTS_DIR / f"real_scores_gpu{part}.csv" for part in range(num_parts)]
    ...
    for part_path in part_paths:
        with open(part_path, newline="") as f:
            for row in csv.DictReader(f):
                rows.append([row["file"], row["delta_min"]])
    ...
    print(f"\nWrote {len(rows)} real scores -> {RESULTS_DIR / 'real_scores.csv'}")
    print("\nDownload real_scores.csv and place it in the project root.")
